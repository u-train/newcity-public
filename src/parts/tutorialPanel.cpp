...
const glm::vec2 tutorialPanelLoc = glm::vec2(0.1, 1.5);
static glm::vec2 lastTutorialPanelLoc = tutorialPanelLoc;
const float tpWidth = 13.0f;
const float tpHeight = 18.5f;
const float tpTitleY = 2.0f;
const float tpTextY = 0.75f;
const glm::vec2 tutorialPanelSize = glm::vec2(tpWidth + tpPadding*2,
...
Part* tutorialArticle(glm::vec2 loc, glm::vec2 size, char** title) {
  Part* scroll = scrollbox(glm::vec2(0,0), size);
  ...
  r(scroll, article(glm::vec2(0,0), glm::vec2(size.x-1, tpTextY),
...
Part* tutorialPanel() {
  glm::vec2 locAdj = tutorialPanelLoc;
  ...
  // Horizontal rule
  //r(result, hr(glm::vec2(tpPadding, yVal), tpWidth-(tpPadding*2.0f)));
  //yVal += tpTitleY;
  ...
  char* title = 0;
  glm::vec2 innerSize = tutorialPanelSize - glm::vec2(tpPadding*2, tpPadding*2+yVal);
  if (ptr->category() != TutorialMenu00) {
    innerSize.y -= tpPadding+buttonHeight*2;
  }
  r(result, tutorialArticle(glm::vec2(0.f, yVal), innerSize, &title));
  ...
  //Part* titlePart = r(result, labelCenter(glm::vec2(1,0),
        //glm::vec2(widthForTitle, titleSpacing),
        //title == 0 ? strdup_s("Tutorial") : title));

  glm::vec2 titleEnd(0,0);
  Part* titlePart = r(result, spanCenter(glm::vec2(1,0), 1,
      glm::vec2(tpWidth-1, titleSpacing),
      title == 0 ? strdup_s("Tutorial") : title, &titleEnd));

  r(result, icon(glm::vec2(0,0), iconQuestion));

  Part* tutExit = button(glm::vec2(tpWidth-1.0f, 0.0f), iconX, closeTutorialPanel);
  r(result, tutExit);
  ...
  if (ptr == 0 || !ptr->tutorialActive()) {
    ...
    Part* tutError = labelCenter(glm::vec2(tpPadding, tutorialPanelSize.y-buttonHeight-(tpPadding*2.0f)),
      glm::vec2(tpWidth-(tpPadding*2.0f), tpTextY),
  ...
  if (ptr->tutorialOKWait()) {
    ...
    */
    ...
    // Ok button
    r(result, labelCenter(glm::vec2(tpPadding, tutorialPanelSize.y-(buttonHeight*2.0f)-(tpPadding*2.0f)),
      glm::vec2(tpWidth-(tpPadding*2.0f), tpTextY),
      strdup_s("Click \"OK\" to continue")));

    Part* tutButt = buttonCenter(glm::vec2(buttonStartX, tutorialPanelSize.y-buttonHeight-(tpPadding*2.0f)), glm::vec2(buttonWidth, buttonHeight),
    ...
    Part* tutStatus = labelCenter(glm::vec2(tpPadding, tutorialPanelSize.y-buttonHeight-(tpPadding*2.0f)),
      glm::vec2(tpWidth-(tpPadding*2.0f), tpTextY),
      strdup_s(tpStatusStr.c_str()));
  ...
  if (ptr->category() != TutorialMenu00) {
    if (ptr->category() != Welcome00) {
      Part* tutBack = r(result, button(glm::vec2(0,1), iconLeft, tutorialBack));
    }

    Part* homeButt = r(result, button(glm::vec2(tpWidth-1,1), iconMenu, glm::vec2(1,1),
