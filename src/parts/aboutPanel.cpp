...
const static glm::vec2 abtSize = glm::vec2(aWidth+abtPad*2, aHeight+abtPad*2);
...
void addContributors(const char* tierFile, const char* tierName,
  ...
  if (file = fopen(fn, "r")) {
    ...
    r(credScroll, label(glm::vec2(abtPad, *credYValue), scale*1.5f, headline));
    *credYValue += scale*2.5f;
    ...
    while(fgets(line, len, file) != NULL) {
      ...
      r(credScroll, label(glm::vec2(x, *credYValue), scale,
...
Part* aboutPanel(float aspectRatio) {
  ...
  Part* result = panel(glm::vec2(uiX,uiGridSizeY)*0.5f - abtSize*0.5f, abtSize);
  ...
  r(result, label(glm::vec2(0,0), 1.5f, strdup_s("About NewCity")));
  r(result, button(glm::vec2(aWidth-1,0), iconX, openMainMenu));
  y += 1.5f;
  //r(result, hr(glm::vec2(0,y), aWidth));
  //y += 0.25;
  startYOffset = y;
  ...
  Part* credScroll = scrollbox(glm::vec2(0,0), glm::vec2(scrollX,(numCredits/2)*scale));
  // Update scale for credits
  scale = 0.8;

  Part* lpWeb = button(glm::vec2(column, credYValue + (scale*1.0f)),
    glm::vec2(13.0f, scale), strdup_s("Follow development at https://lonepine.io"), linkCallback);
  lpWeb->itemData = AboutLink::LP_WEB;
  Part* lpRed = button(glm::vec2(column, credYValue + (scale*2.0f)),
    glm::vec2(13.0f, scale), strdup_s("Reddit - r/NewCity"), linkCallback);
  lpRed->itemData = AboutLink::LP_REDDIT;
  Part* lpDis = button(glm::vec2(column, credYValue + (scale*3.0f)),
    glm::vec2(13.0f, scale), strdup_s("Join the discussion on Discord"), linkCallback);
  lpDis->itemData = AboutLink::LP_DISCORD;
  Part* lpSte = button(glm::vec2(column, credYValue + (scale*4.0f)),
    glm::vec2(13.0f, scale), strdup_s("Check out NewCity's Steam Store page"), linkCallback);
  lpSte->itemData = AboutLink::LP_STEAM;
  Part* lpTwi = button(glm::vec2(column, credYValue + (scale*5.0f)),
    glm::vec2(13.0f, scale), strdup_s("Twitter - @lone_pine_games"), linkCallback);
  lpTwi->itemData = AboutLink::LP_TWITTER;
  Part* lpYou = button(glm::vec2(column, credYValue + (scale*6.0f)),
    glm::vec2(13.0f, scale), strdup_s("Follow Lone Pine Games on YouTube"), linkCallback);
  lpYou->itemData = AboutLink::LP_YOUTUBE;
  Part* lpTwc = button(glm::vec2(column, credYValue + (scale*7.0f)),
    glm::vec2(13.0f, scale), strdup_s("Follow Lone Pine Games on Twitch"), linkCallback);
  lpTwc->itemData = AboutLink::LP_TWITCH;
  Part* lpFB = button(glm::vec2(column, credYValue + (scale*8.0f)),
    glm::vec2(13.0f, scale), strdup_s("Like NewCity on Facebook"), linkCallback);
  lpFB->itemData = AboutLink::LP_FACEBOOK;
  Part* lpEm = button(glm::vec2(column, credYValue + (scale*9.0f)),
    glm::vec2(13.0f, scale), strdup_s("Support - lonepinegames@gmail.com"), linkCallback);
  ...
  r(credScroll, label(glm::vec2(abtPad, credYValue), scale*1.5f,
    strdup_s("Lone Pine Games:")));
  credYValue += scale*1.5f+scale;

  r(credScroll, label(glm::vec2(abtPad*2, credYValue), scale*1.1f,
    strdup_s("Creator, Lead Developer\n")));
  credYValue += scale*1.1f;
  r(credScroll, label(glm::vec2(abtPad*2, credYValue), scale,
  ...
  r(credScroll, hr(glm::vec2(abtPad, credYValue), scrollX-(abtPad*6)));
  credYValue += scale;
  ...
  r(credScroll, label(glm::vec2(abtPad, credYValue), scale*1.5f,
    strdup_s("Technology:")));
  r(credScroll, multiline(glm::vec2(column, credYValue), glm::vec2(column, scale),
    strdup_s(
  ...
  */
  ...
  Part* objLicButt = button(glm::vec2(abtPad*2, credYValue), glm::vec2(23.0f, scale),
    strdup_s("Full text - "
  ...
  for(int m = 0; mitCredits[m][0] != '\0'; m++) {
    r(credScroll, label(glm::vec2(abtPad, credYValue), scale,
  ...
  */
  ...
  for (int i = 0; i < numCredits; i++) {
    r(credScroll, label(glm::vec2(abtPad*2 + (i%2)*column, credYValue+(i/2)*scale),
          scale, strdup_s(credits[i])));
  }
  credYValue += ((numCredits+1)/2)*scale;
  r(credScroll, label(glm::vec2(1+((numCredits-1)%2)*column, credYValue),
  ...
  r(credScroll, hr(glm::vec2(abtPad, credYValue), scrollX-(abtPad*6)));
  credYValue += scale;
  r(credScroll, multiline(glm::vec2(abtPad*2, credYValue), glm::vec2(aWidth-scale, scale),
    strdup_s(
  ...
  r(result, scrollboxFrame(glm::vec2(0,y), glm::vec2(scrollX,scrollY),
