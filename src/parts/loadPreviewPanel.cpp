...
Part* loadPreviewPanel(glm::vec2 loc, glm::vec2 size, const char* filename) {
  ...
  if (fileExists(imageFile)) {
    Part* img = r(result, image(glm::vec2(-prevPad, -prevPad), size.x,
  ...
  */
  ...
  if (design) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      r(result, icon(glm::vec2(x+2, y), glm::vec2(1,1), iconBusinessType[i]));
      x+=3;
    }
    if (x > 0) y ++;
    x = 0;

    r(result, labelRight(glm::vec2(0, y), glm::vec2(1,1),
          sprintf_o("%d", int(designPreview.minDensity*10))));
    r(result, icon(glm::vec2(1, y), glm::vec2(1,1), iconDensity));
    r(result, labelRight(glm::vec2(2, y), glm::vec2(1,1),
          sprintf_o("%d", int(designPreview.minLandValue*10))));
    r(result, icon(glm::vec2(3, y), glm::vec2(1,1), iconLandValue));
    y ++;

    r(result, label(glm::vec2(0,y), 1, sprintf_o("%d-%d", designPreview.minYear,
    ...
    for (int i = 0; i < numEffects; i++) {
      ...
      r(result, icon(glm::vec2(x+2, y), glm::vec2(1,1), getEffectIcon(i)));
      x+=3;
      if (x > 11) {
        y ++;
        x = 0;
    ...
    if (designPreview.zone == GovernmentZone) {
      r(result, label(glm::vec2(0,y), 1, printMoneyString(designPreview.cost)));
      char* perYear = printMoneyString(designPreview.maintenance);
      r(result, label(glm::vec2(3,y), 1, sprintf_o("%s/yr", perYear)));
      free(perYear);
      y++;
    }
  }
