...
void effectIcon(Part* pnl, glm::vec2 loc, item effect, float val) {
  float x = loc.x;
  float y = loc.y;
  int number = abs(val);
  bool negative = val < 0;
  ...
  Part* numLabel = labelRight(glm::vec2(x, y), glm::vec2(numberSpace, 0.75),
    sprintf_o("%s%d", negative ? "-" : "+", number));
  r(pnl, numLabel);

  Part* effIcon = icon(glm::vec2(x+numberSpace, y), glm::vec2(0.75, 0.75),
...
bool openCitipediaEffectPage(Part* part, InputEvent event) {
  ...
  std::string article = getEffectString(effect);
...
bool openCitipediaHomelessnessPage(Part* part, InputEvent event) {
  std::string article = "concepts/homeless";
  followLink(strdup_s(article.c_str()));
  return true;
}
...
Part* amenityInfoPart(glm::vec2 loc, glm::vec2 size,
  ...
  r(pnl, label(glm::vec2(0,0), 1, strdup_s(d->displayName)));
  //r(pnl, hr(glm::vec2(0,1), pnl->dim.end.x-pnl->padding*2));
  float y = 1.1;
  float padding = 0.1;
  ...
  if (d->zone != GovernmentZone) {
    ...
    r(pnl, label(glm::vec2(0,y), 1, sprintf_o("%s", costStr), cost > credit));
    free(costStr);
    return pnl;
  }
  ...
  Part* costLbl = r(pnl, label(glm::vec2(0,y), 1, sprintf_o("%s,", costStr),
        unaffordableRed && cost > credit));
  Part* maintLbl = r(pnl, label(glm::vec2(costStrWidth,y), 1, sprintf_o("%s/year", maintStr), unaffordableRed && maintenance > earnings));
  free(costStr);
  free(maintStr);
  ...
  if (d->flags & _designSingleton) {
    r(pnl, label(glm::vec2(8.6, y+0.2), 0.75, strdup_s("Limit one per city")));
  }

  y += 1.2;

  std::string imageFile = "designs/";
  ...
  glm::vec2 imgDim = getTextureDimensions(imageFile.c_str());
  if (imgDim.y > 0) {
    ...
    Part* img = r(pnl, image(glm::vec2(0,y), imgSizeX,
          strdup_s(imageFile.c_str()), &sizeY));
    img->dim.start.z -= 0.5f;
    x += imgSizeX + padding;
  }

  amenityStats(pnl, glm::vec2(x,y),
      glm::vec2(pnl->dim.end.x-x, pnl->dim.end.y-y), designNdx);
...
void amenityStats(Part* pnl, glm::vec2 loc, glm::vec2 size, item designNdx) {
  ...
  for (int i = 0; i < dots.size(); i++) {
    ...
    item effect = dots[i];
    Part* dot = r(pnl, greenBlock(glm::vec2(x, dotY), glm::vec2(dotSizeX, dotSizeY)));
    ...
    if (effect < 0) {
      ...
      r(dot, block(glm::vec2(dotSizeX-littleDotSize, 0.75-littleDotSize)*.5f,
            glm::vec2(littleDotSize, littleDotSize)));
    } else {
      effectIcon(dot, glm::vec2(0,0), effect, d->effect[effect]);
    }
  ...
  r(pnl, sunkenBlock(glm::vec2(x-0.05,y), glm::vec2(0.1, size.y-padding*2)));
  x += 0.1;
  ...
  if (currentEffect >= 0) {
    ...
    if (val != 0) {
      ...
      //effectIcon(pnl, glm::vec2(0,y), currentEffect, val);
      Part* nameLabel = labelCenter(glm::vec2(x, y), glm::vec2(spaceX-titleScale, titleScale), strdup_s(getEffectString(currentEffect)));
      r(pnl, nameLabel);

      Part* citipediaButt = r(pnl, button(glm::vec2(x+spaceX-titleScale,y), iconCitipedia, glm::vec2(titleScale,titleScale), openCitipediaEffectPage, currentEffect));
      citipediaButt->onHover = preventAmenityInfoDotAdvance;
      y += titleScale;
      //r(pnl, hr(glm::vec2(x,y), pnl->dim.end.x-pnl->padding*2-x));
      //y += 0.2;

      char* desc = getEffectDescriptor(currentEffect, val, d->flags);
      Part* descLabel = spanCenter(glm::vec2(x, y), x,
          glm::vec2(spanSpace, smallTxt), desc, &y);
      r(pnl, descLabel);
    ...
    if (currentEffect == Health && healthCare) {
      r(pnl, spanCenter(glm::vec2(x, y), x,
          glm::vec2(spanSpace, smallTxt), strdup_s("Provides Health Care"), &y));
    }

    if (currentEffect == EducationEffect) {
      if (hsDiploma) {
        r(pnl, spanCenter(glm::vec2(x, y), x, glm::vec2(spanSpace, smallTxt),
              strdup_s("Grants High School Diplomas"), &y));
      }

      if (bclDegree) {
        r(pnl, spanCenter(glm::vec2(x, y), x,
            glm::vec2(spanSpace, smallTxt), strdup_s("Grants Bachelor's Degrees"), &y));
      }

      if (phd) {
        r(pnl, spanCenter(glm::vec2(x, y), x,
            glm::vec2(spanSpace, smallTxt), strdup_s("Grants Doctorates"), &y));
      }
    }
    ...
    if (anyFam) {
      ...
      r(pnl, labelRight(glm::vec2(x, y), glm::vec2(strX,1), famStr));
      r(pnl, icon(glm::vec2(x+2, y), glm::vec2(1,1),
    ...
    for (int i = 0; i < 4; i++) {
      ...
      r(pnl, labelRight(glm::vec2(x, y), glm::vec2(strX,1), bizStr));
      r(pnl, icon(glm::vec2(x+strX, y), glm::vec2(1,1), iconBusinessType[i]));
      x+=strX+1+padding;
    }
    ...
    if (!isHotel && getDesignNumHomes(designNdx)) {
      Part* citipediaButt = r(pnl, button(glm::vec2(spanSpace-smallTxt,y), iconCitipedia, glm::vec2(smallTxt,smallTxt), openCitipediaHomelessnessPage, 0));
      r(pnl, spanCenter(glm::vec2(x, y), x, glm::vec2(spanSpace-smallTxt, smallTxt), sprintf_o("Provides %d Homes for homeless families.", numHomes), &y));
    }
    ...
    if (aquatic) {
      r(pnl, spanCenter(glm::vec2(x, y), x,
          glm::vec2(spanSpace, smallTxt), strdup_s("Must be Placed on a Coast"), &y));
    }

    if (noEmDom) {
      r(pnl, spanCenter(glm::vec2(x, y), x,
          glm::vec2(spanSpace, smallTxt),
