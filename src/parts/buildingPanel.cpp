...
void buildingPanel(Part* result, Building* b) {
  ...
  glm::vec3 ico = iconZoneMono[d->zone];
  ...
  if (selectionNameState.flags & _textBoxEditing) {
    ...
    y += 1-scl;
    glm::vec2 end;
    r(result, span(glm::vec2(sclPad+spPadding,y), 0, glm::vec2(spWidth-sclPad*2, scl),
        strdup_s(name), &end));
    y = end.y;
    if (end.x > 0) y += scl;
  }
  ...
  r(result, label(glm::vec2(0,y-.25f), 0.75, ndxStr));

  // HR
  r(result, hr(glm::vec2(ndxStrWidth,y), spWidth-ndxStrWidth));
  y += 0.25;

  Part* followButt = button(glm::vec2(spWidth-1,y), iconEye, followSelection);
  ...
  if (isFeatureEnabled(FNoteObject)) {
    Part* butt = button(glm::vec2(spWidth-2,y), iconPin, glm::vec2(1,1),
  ...
  for (int i = 0; i < numBuildingPanelTabs; i++) {
    float tabButtWidth = stringWidth(buildingPanelTabLabels[i])*.65 +
      spPadding*2;
    Part* tab = r(result, button(glm::vec2(x,y), glm::vec2(tabButtWidth, 0.85),
  ...
  if (buildingPanelTab == BuildingOverviewTab) {
    Part* inner = scrollbox(glm::vec2(0,0),
        glm::vec2(ipWidth + spPadding*2, ipHeight + spPadding*2));
    //Part* inner = panel(glm::vec2(0,y),
        //glm::vec2(ipWidth + spPadding*2, ipHeight + spPadding*2));
    ...
    r(inner, graphElementButton(glm::vec2(0,y), b->graphLoc));
    y += sclPad;

    if (!(b->flags & _buildingComplete)) {
      r(inner, label(glm::vec2(0, y), scl, strdup_s("Planned")));
      return;
    }

    r(inner, label(glm::vec2(0, y), scl, sprintf_o("Built %s",
            printDateString(b->builtTime))));
    y += sclPad;

    if (d->zone != GovernmentZone &&
        b->flags & _buildingAbandoned) {
      r(inner, labelRed(glm::vec2(0,y), 1, strdup_s("Abandoned")));
      y += sclPad;
    }
    y++;
    ...
    if (d->flags & _designHasStatue) {
      ...
      for (int i = 0; i < d->decos.size(); i++) {
        ...
        if (deco.decoType == statueDecoType()) {
          ...
          r(inner, label(glm::vec2(scl, y+sclPad*numStatues), scl,
                strdup_s(getStatueName(statueNdx))));
        }
      }

      if (numStatues > 0) {
        r(inner, label(glm::vec2(0, y), scl, strdup_s(
    ...
    if (d->zone == GovernmentZone) {
      ...
      amenityStats(inner, glm::vec2(0,y), glm::vec2(ipWidth-0.5, statsSizeY), b->design);
      y += statsSizeY + 1;
      ...
      for (int i = 0; i < numEffects; i++) {
        ...
        r(inner, icon(glm::vec2(1, y), glm::vec2(0.75, 0.75), getEffectIcon(i)));
        Part* numLabel = labelRight(glm::vec2(0, y+0.125), glm::vec2(1, 0.75),
          sprintf_o("%s%d", negative ? "-" : "+", number));
        r(inner, numLabel);

        Part* nameLabel = label(glm::vec2(1.75, y+0.125), 0.75,
        ...
        r(inner, label(glm::vec2(4.75, y+0.125), 0.75, desc));
        y += sclPad;
      }
      y ++;
      */
      ...
      for (int i = 0; i < numEffects; i++) {
        ...
        glm::vec3 ico = negative ? iconRedTile : iconGreenTile;

        r(inner, icon(glm::vec2(xOff, y), getEffectIcon(i)));
        r(inner, label(glm::vec2(xOff + 1, y+0.125), 0.75,
          sprintf_o("%s%d %s", negative ? "-" : "+",
            number, getEffectString(i))));

        for (int j = 0; j < number; j++) {
          glm::vec2 loc = glm::vec2(xOff + int(j/2)*0.5, y + (j%2)*0.5 + 0.0);
          glm::vec2 size = glm::vec2(0.5, 0.5);
    ...
    Part* heatmapPanel = panel(
        glm::vec2((ipWidth-hmWidth)*.5f, y),
        glm::vec2(numHeatMaps*1.1f+.2f, hmBase + 1.3f));
    ...
    for (int i=0; i < numHeatMaps; i++) {
      ...
      Part* hmButt = panel(glm::vec2(hx, 0.0f),
          glm::vec2(1.0f, heatmapPanel->dim.end.y-(hmPad*2)));
      ...
      r(hmButt, icon(glm::vec2(0.0f, hmBase+hmPad), glm::vec2(1,1), iconHeatmap[i]));
      for (int j = 0; j < hmValI && j < 10; j++) {
        //float tileSize = 1.f;//(j+1 >= value) ? 1.f : (value - j);
        //float tileSize = glm::clamp(value - j, 0.f, 1.f);
        float tileSize = 1.f;
        tileSize *= .5f;
        r(hmButt, icon(
            glm::vec2(0.0f+(j%2)*.5, hmBase - (j/2+1)*.5 + .5-tileSize),
            glm::vec2(.5, tileSize), iconHeatmapColor[i]));
      }
    ...
    r(inner, label(glm::vec2(0, y), scl, sprintf_o("Design file: %s",
            d->name)));
    y += sclPad;

    Part* designButt = r(inner, button(glm::vec2(0,y), iconWrench, glm::vec2(ipWidth,scl),
        strdup_s("Edit Design"), setFixBuildingMode, 1));
    setPartTooltipValues(designButt, DesignerFixBuilding);
    y += sclPad;

    /*
    Part* designButt = r(inner, button(glm::vec2(0,y), iconCash, glm::vec2(ipWidth,scl),
    ...
    */
    ...
    if (b->entity != 0) {
      ...
      glm::vec2 end;
      r(inner, span(glm::vec2(0.125, y+.125), scl, glm::vec2(ipWidth, scl-.125),
            sprintf_o("Texture file:\n%s", getTextureFilename(texNdx)), &end));
      y = end.y + sclPad;
    }

    r(inner, label(glm::vec2(0, y), scl, sprintf_o("Min Density: %d",
            int(d->minDensity*10))));
    y += sclPad;

    r(inner, label(glm::vec2(0, y), scl, sprintf_o("Min Value: %d",
            int(d->minLandValue*10))));
    y += sclPad;

    r(inner, label(glm::vec2(0, y), 1, strdup_s(" "))); // to force scrollbox padding

    Part* sbFrame = r(result, scrollboxFrame(glm::vec2(0,scrollboxY),
        glm::vec2(ipWidth + spPadding*2, ipHeight + spPadding*2),
  ...
  if (buildingPanelTab == BuildingFinancialTab) {
    ...
    if (d->zone == GovernmentZone) {
      ...
      r(inner, label(glm::vec2(0,y), scl,
            sprintf_o("%s value", value)));
      free(value);
      y += sclPad;

      if (isFeatureEnabled(FSellBuilding)) {
        Part* sellButt = button(glm::vec2(0, y), iconCash, glm::vec2(7, scl),
      ...
      if (enabled) {
        ...
        r(inner, labelRed(glm::vec2(0,y), scl, strdup_s("Shutdown")));
      }
      y += sclPad;

      if (isFeatureEnabled(FShutDownBuilding)) {
        Part* shutButt = button(glm::vec2(0,y),
          enabled ? iconClosed : iconOpen, glm::vec2(5,scl),
      ...
      Part* historicalButt = button(glm::vec2(0,y),
  ...
  if (buildingPanelTab == BuildingWhatsInsideTab) {
    ...
    float x = 0;
    r(inner, labelRight(glm::vec2(x, y), glm::vec2(scl,scl),
          sprintf_o("%d", b->peopleInside.size())));
    r(inner, icon(glm::vec2(x+scl, y), glm::vec2(scl,scl), iconPersonMan));
    x+=2;

    item numHomes = getDesignNumHomes(b->design);
    item numFams = b->families.size();
    if (numHomes > 0 || numFams > 0) {
      r(inner, labelRight(glm::vec2(x, y), glm::vec2(scl*3,scl),
            sprintf_o("%d/%d", numFams, numHomes)));
      r(inner, icon(glm::vec2(x+scl*3, y), glm::vec2(scl,scl),
    ...
    for (int i = 0; i < 5; i++) {
      ...
      r(inner, icon(glm::vec2(x+scl*3, y), glm::vec2(scl,scl), iconBusinessType[i]));
      x+=4*scl;
    }

    y++;

    r(inner, objectList(glm::vec2(0,y),
          glm::vec2(ipWidth,ipHeight-y),
...
//void buildingPanel(Part* result, Building* selection) {
  ...
  float y = 0;
  float scrollBoxTop = 8;
  glm::vec3 ico = iconZoneMono[selection->zone];
  ...
  // HR
  r(result, hr(glm::vec2(ndxStrWidth,1.0f), spWidth-ndxStrWidth));
  y += 0.2;

  r(result, graphElementButton(glm::vec2(0,y), selection->graphLoc));
  y += sclPad;

  if (!(selection->flags & _buildingComplete)) {
    r(result, label(glm::vec2(0, y), scl, strdup_s("Planned")));
    return;
  }

  r(result, label(glm::vec2(0, y), scl, sprintf_o("Built %s",
          printDateString(selection->builtTime))));
  y += sclPad;

  r(result, label(glm::vec2(0, y), 0.7, sprintf_o("Design file: %s",
          d->name)));
  y += sclPad;

  if (selection->entity != 0) {
    item texNdx = getEntity(selection->entity)->texture;
    r(result, label(glm::vec2(0, y), 0.6, sprintf_o("Texture file:\n%s",
  ...
  if (selection->zone == GovernmentZone) {
    ...
    if (enabled) {
      ...
      r(result, labelRed(glm::vec2(0,y), scl, strdup_s("Shutdown")));
    }

    if (isFeatureEnabled(FShutDownBuilding)) {
      Part* shutButt = button(glm::vec2(5,y),
        enabled ? iconClosed : iconOpen, glm::vec2(5,1),
        strdup_s(enabled ? "Shutdown" : "Reopen"),
        toggleEnableGovBuilding, 0);
      r(result, shutButt);
      if (blinkFeature(FShutDownBuilding)) {
    ...
    if (isFeatureEnabled(FSellBuilding)) {
      Part* sellButt =button(glm::vec2(5,y), iconCash, glm::vec2(7,1),
    ...
    Part* historicalButt = button(glm::vec2(5.5f,y),
        historical ? iconBusiness : iconNull,
        glm::vec2(6.5f,scl),
        strdup_s(historical ? "Historical Building" : "Mark Historical"),
        toggleBuildingHistorical, 0);
    ...
    if(selection->lots.size() > 0) {
      ...
      r(result, label(glm::vec2(6, y), scl,
        sprintf_o("Max Lot Density: %d", maxDens)));
    }
  ...
  if (selection->zone != GovernmentZone &&
    ...
    r(result, labelRed(glm::vec2(0,y), 1, strdup_s("Abandoned")));
    y += sclPad;
  }
  ...
  if (selection->zone == GovernmentZone) {
    for (int i = 0; i < numEffects; i++) {
      ...
      r(result, icon(glm::vec2(1, y), glm::vec2(0.75, 0.75), getEffectIcon(i)));
      Part* numLabel = labelRight(glm::vec2(0, y+0.125), glm::vec2(1, 0.75),
        sprintf_o("%s%d", negative ? "-" : "+", number));
      r(result, numLabel);
      ...
      char* desc = getEffectDescriptor(i, val, d->flags);
      r(result, label(glm::vec2(4.75, y+0.125), 0.75, desc));
      y += sclPad;
    }
    ...
    for (int i = 0; i < numEffects; i++) {
      ...
      r(result, icon(glm::vec2(xOff, y), getEffectIcon(i)));
      r(result, label(glm::vec2(xOff + 1, y+0.125), 0.75,
        sprintf_o("%s%d %s", negative ? "-" : "+",
          number, getEffectString(i))));
  ...
  float ins = .8f;
  float ipad = .1f;
  glm::vec2 icoSize(iys-ipad*2, iys-ipad*2);
  ...
  r(result, labelCenter(glm::vec2(ix, y-iys), glm::vec2(ins, tys),
        sprintf_o("%d", numPop)));
  r(result, icon(glm::vec2(ix,y+ipad), icoSize, iconPersonMan));
  ix += ixs;

  if (numBiz > 0 || maxBiz > 0) {
    r(result, labelCenter(glm::vec2(ix, y-iys), glm::vec2(ins, tys),
          sprintf_o("%d/%d", numBiz, maxBiz)));
    r(result, icon(glm::vec2(ix,y+ipad), icoSize, iconBusiness));
  }
  ix += ixs;
  ...
  if (numFam > 0 || maxFam > 0) {
    ...
    r(result, icon(glm::vec2(ix,y+ipad), icoSize, iconFamily));
  }
  ix += ixs;
  ...
  float hmPad = 0.1f;
  Part* heatmapPanel = panel(
      glm::vec2(spWidth-numHeatMaps*1.1f-.2f, scrollBoxTop-hmBase-1.4f),
      glm::vec2(numHeatMaps*1.1f+.2f, hmBase + 1.3f));
  heatmapPanel->padding = hmPad;
  heatmapPanel->flags |= _partLowered;
  ...
  for (int i=0; i < numHeatMaps; i++) {
    ...
    Part* hmButt = panel(glm::vec2(hx, 0.0f), glm::vec2(1.0f, heatmapPanel->dim.end.y-(hmPad*2)));
    hmButt->flags |= _partHover;
    hmButt->itemData = HeatMapIndex::Pollution+i;
    hmButt->onClick = setHeatMap;
    hmButt->renderMode = RenderTransparent;
  ...
  r(result, objectList(glm::vec2(0,scrollBoxTop),
        glm::vec2(spWidth,spHeight-scrollBoxTop),
