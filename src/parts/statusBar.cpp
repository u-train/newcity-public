...
Part* statusBar(float uiX) {
  float padding = 0.1;
  Part* statusBar = part(line(glm::vec3(0,0,0), glm::vec3(25, 1+padding*2, 0)));
  ...
  if (game) {
    ...
    Part* nameBar = r(statusBar,
        panel(glm::vec2(x,0), glm::vec2(nameBarX, 1+fullCityNameBar+extraY+padding*2)));
    ...
    if (fullCityNameBar) {
      ...
      Part* nameTB = r(nameBar, textBoxLabel(glm::vec2(0,0),
            glm::vec2(nameLength-padding, 1), &cityNameTBState));
      ...
      r(nameBar, label(glm::vec2(0,1), 0.85, diffLbl));

      // Height seed
      Part* hSeedCon = panel(glm::vec2(0.0f, 2.0f), glm::vec2(nameLength, 1.f));
      hSeedCon->renderMode = RenderTransparent;
      setPartTooltipValues(hSeedCon, TooltipType::GenSeedH);

      Part* hSeedLbl = label(glm::vec2(0.0f, 0.0f), 0.85f,
        (char*)hSeedStr);
      ...
      r(nameBar, label(glm::vec2(0,0), 1, strdup_s(getCityName())));
      free(hSeedStr);
    }
    ...
    if (editing) {
      Part* nameTB = r(nameBar,
          textBox(glm::vec2(0,0), glm::vec2(nameLength-1-padding, 1), &cityNameTBState));
      nameTB->onCustom = stopEditingCityName;
      r(nameBar, button(glm::vec2(nameLength-1, 0), iconCheck,
            glm::vec2(1, 1), stopEditingCityName, 0));
    } else {
      r(nameBar, label(glm::vec2(0,0), 1, strdup_s(getCityName())));
    }
    */
  ...
  // Time and Date Bar
  if (game) {
    Part* timeBar = panel(glm::vec2(x,0), glm::vec2(10, 1+padding*2));
    timeBar->padding = padding;
    float time = getCurrentDateTime();
    r(timeBar, label(glm::vec2(0,0), 1,
          printDateTimeStringSanYear(getCurrentDateTime())));
    int year = getCurrentYear();
    r(timeBar, labelRight(glm::vec2(7-padding*2,0), glm::vec2(3,1),
          sprintf_o("%d", year)));
    r(statusBar, timeBar);
    x += 10.5;
  }
  ...
  if (game && isFeatureEnabled(FPopulation)) {
    ...
    Part* popBar = panel(glm::vec2(x,0), glm::vec2(popBarSize+padding*2, 1+padding*2));
    ...
    r(popBar, icon(glm::vec2(0,0), glm::vec2(1,1), iconPersonMan));
    r(popBar, label(glm::vec2(1,0), 1, popString));
    r(statusBar, popBar);
  ...
  if (!designer && !isFixBuildingMode() && isFeatureEnabled(FBudget)
    ...
    Part* moneyBar = panel(glm::vec2(x,0),
        glm::vec2(moneyBarWidth+padding*2, 1+padding*2));
    ...
    if (getLeftPanel() == BudgetPanel) {
      //glm::vec3 gray = getColorInPalette(PickerPalette::GrayDark);
    ...
    */
    ...
    Part* moneyLbl = r(moneyBar, label(glm::vec2(0,0), 1, moneyString));
  ...
  /*
  if (testMode) {
    Part* titleBar = panel(glm::vec2(x,0),
        glm::vec2(7, 1+padding*2));
    titleBar->padding = padding;
    r(titleBar, label(glm::vec2(0,0), 1, strdup_s("Test Mode")));
  ...
  */
  ...
  if (!game) {
    ...
    Part* timeBar = panel(glm::vec2(x,0),
        glm::vec2(timeSizeX+padding*2,timeSizeY+padding*2));
    timeBar->padding = padding;
    r(timeBar, slider(glm::vec2(0,0), glm::vec2(timeSizeX,timeSizeY),
      getLightTime(), setTime));

    // Icons representing night/day pos on slider
    Part* ico = r(timeBar, icon(glm::vec2(0,0), glm::vec2(1,1),
      iconWeatherMoon));
    ico->flags |= _partTransparent;
    ico = r(timeBar, icon(glm::vec2(2,0), glm::vec2(1,1),
      iconWeatherSun));
    ico->flags |= _partTransparent;
    ico = r(timeBar, icon(glm::vec2(4,0), glm::vec2(1,1),
      iconWeatherMoon));
    ico->flags |= _partTransparent;
    ...
    Part* seasonBar = panel(glm::vec2(x,0),
        glm::vec2(timeSizeX+padding*2,timeSizeY+padding*2));
    seasonBar->padding = padding;
    r(seasonBar, slider(glm::vec2(0,0), glm::vec2(timeSizeX,timeSizeY),
      getLightSeason()*.25f, setSeason));

    //ico = r(seasonBar, icon(glm::vec2(0,0), glm::vec2(1,1),
      //iconWeatherCold));
    //ico->flags |= _partTransparent;
    ico = r(seasonBar, icon(glm::vec2(2,0), glm::vec2(1,1),
      iconWeatherSun));
    ico->flags |= _partTransparent;
    ico = r(seasonBar, icon(glm::vec2(4,0), glm::vec2(1,1),
      iconWeatherCold));
    ico->flags |= _partTransparent;
  ...
  // Number of Cars
  if (testMode) {
    Part* rateBar = panel(glm::vec2(x,0), glm::vec2(8+padding*2, 1+padding*2));
    rateBar->padding = padding;
    //r(rateBar, label(glm::vec2(0,0), 1, sprintf_o("%'d Cars, %'d/Hour",
            //countVehicles(), int(getVehicleRate()/6))));
    r(rateBar, labelRight(glm::vec2(0,0), glm::vec2(7,1),
          sprintf_o("%d", countVehicles())));
    r(rateBar, icon(glm::vec2(7,0), glm::vec2(1,1), iconCar));
  ...
  if (testMode) {
    ...
    Part* deleteBar = panel(glm::vec2(x,0), glm::vec2(delW+padding*2, 1+padding*2));
    deleteBar->padding = padding;
    r(deleteBar, button(glm::vec2(0,0), glm::vec2(delW,1), strdup_s(delS),
          deleteAllCars));
  ...
  if (game && getAspectRatio() > 1.4 &&
    ...
    Part* unempBar = panel(glm::vec2(x,0), glm::vec2(6+padding*2, 1+padding*2));
    ...
    r(unempBar, label(glm::vec2(0,0), 1, sprintf_o("%2d%% Unemployed",
  ...
  */
  ...
  if (game && getAspectRatio() > 1.4 &&
    ...
    Part* weatherBar = panel(glm::vec2(x,0), glm::vec2(2.5f+padding*2, 1+padding*2));
    r(statusBar, weatherBar);
    x += 3.0f + padding*2;
    weatherBar->padding = padding;
    float tempSize = 0.8;
    r(weatherBar, label(glm::vec2(0.1,0.1), tempSize,
          useMetric() ? sprintf_o("%2dC", int(w.temp)) :
          sprintf_o("%2dF", int(w.temp*1.8 + 32))));

    glm::vec3 ico = getWeatherIconVec3();
    r(weatherBar, icon(glm::vec2(1.5f,0.f), ico));
  }
  ...
  if (!designer &&
    ...
    Part* speedBar = panel(glm::vec2(x, 0),
      glm::vec2(size+padding*2, 1+padding*2));
    x += size + .5 + padding*2;
    ...
    for (int i=0; i < size; i++) {
      Part* butt = button(glm::vec2(i,0), iconGameSpeed[i>4?4:i], speedButton, i);
