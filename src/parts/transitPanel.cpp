...
Part* transitPanel() {
  ...
  glm::vec2 tpLoc = glm::vec2(0.1, 1.5);
  ...
  r(result, button(glm::vec2(tpWidth-1,0), iconX, glm::vec2(1,1), closeTransitPanel, 0));

  if (currentSystem == 0) {
    r(result, icon(glm::vec2(0,y), glm::vec2(1,1), iconBus));
    r(result, label(glm::vec2(1,y), 1, strdup_s("Transit")));
    y += 1;
    r(result, hr(glm::vec2(0,y), tpWidth));
    y += 0.25;

    if (isFeatureEnabled(FTransitSystems)) {
      r(result, button(glm::vec2(0,y), iconPlus, buttSize,
    ...
    Part* sbox = scrollbox(glm::vec2(0,0), glm::vec2(tpWidth-1, scrollBoxHeight));
    ...
    for (int i=1; i <= sizeTransitSystems(); i++) {
      ...
      r(sbox, button(glm::vec2(0,ys), iconTransitLogos[system->logo], buttSize,
        strdup_s(system->name), setCurrentTransitSystem, i));
      glm::vec3 xColor = getColorInPalette(system->color[0]);
      r(sbox, gradientBlock(glm::vec2(tpWidth-1.5,ys+.125),
            glm::vec2(.5,.5), xColor, xColor));
      xColor = getColorInPalette(system->color[1]);
      r(sbox, gradientBlock(glm::vec2(tpWidth-1.0,ys+.125),
            glm::vec2(.5,.5), xColor, xColor));
      //r(sbox, button(glm::vec2(tpWidth-1.5,ys), iconTrash, glm::vec2(1,1),
            //removeLine, i));
      ys ++;
    }

    r(result, scrollboxFrame(glm::vec2(0,y),
          glm::vec2(tpWidth, scrollBoxHeight), &systemsScroll, sbox));

  } else if (currentLine == 0) {
    TransitSystem* system = getTransitSystem(currentSystem);
    r(result, icon(glm::vec2(0,y), glm::vec2(1,1), iconTransitLogos[system->logo]));
    r(result, label(glm::vec2(1,y), 1, strdup_s(system->name)));
    r(result, button(glm::vec2(tpWidth-2,y), iconPencil, glm::vec2(1,1),
          toggleEditingTransitBrand, 0));
    y += 1;
    r(result, hr(glm::vec2(0,y), tpWidth));
    y += 0.25;

    r(result, button(glm::vec2(0,y), iconLeft, glm::vec2(tpWidth,0.75),
          strdup_s("All Transit Systems"), setCurrentTransitSystem, 0));
    y += 1.25;

    //r(result, button(glm::vec2(0,y), iconX, glm::vec2(tpWidth,0.75),
    ...
    for (int i = 0; i < 2; i++) {
      r(result, icon(glm::vec2(0,y), glm::vec2(0.75,0.75), iconCash));
      float val = i ? system->transferPrice : system->ticketPrice;
      float sliderVal = val / getInflationMultiplier();
      Part* slide = r(result, slider(glm::vec2(0.75,y), glm::vec2(slideWidth, 0.75),
            sliderVal, setTicketPrice));
      slide->itemData = i;
      r(result, label(glm::vec2(slideWidth+1,y), 0.75,
    ...
    for (int i=1; i <= sizeTransitLines(); i++) {
      ...
      glm::vec3 xColor = getTransitLineColorInPalette(i);
      r(sbox, gradientBlock(glm::vec2(tpWidth-3.625,ys+.125),
            glm::vec2(.5,.5), xColor, xColor));
      r(sbox, button(glm::vec2(tpWidth-1.5,ys), iconTrash, glm::vec2(1,1),
            removeLine, i));
      ys ++;
    ...
    r(result, scrollboxFrame(glm::vec2(0,y),
          glm::vec2(tpWidth, scrollBoxHeight), &linesScroll, sbox));
    ...
    r(result, icon(glm::vec2(0,y), glm::vec2(1,1), iconTransitLogos[system->logo]));
    r(result, textBoxLabel(glm::vec2(1,y), glm::vec2(tpWidth-2,1), &transitNameText));
    y += 1;
    r(result, hr(glm::vec2(0,y), tpWidth));
    y += 0.25;
    ...
    r(result, colorPicker(glm::vec2(tpWidth - colorPickerWidth, y),
          colorPickerWidth, line->color, setTransitLineColor, 0));
    glm::vec3 xColor = getColorInPalette(system->color[0]);

    // Color Picker Default Button
    Part* defaultButt = r(result, button(glm::vec2(tpWidth-colorPickerWidth,y+2),
          iconNull, glm::vec2(colorPickerWidth,0.7),
          strdup_s("Use Default"), setTransitLineColor, 255));
    defaultButt->flags |= _partIsPanel;
    if (line->color == 255) {
      defaultButt->flags |= _partHighlight;
    }
    r(defaultButt, gradientBlock(glm::vec2(0,0), glm::vec2(0.5, 0.5), xColor, xColor));

    // Color Picker Background
    //Part* background = r(result,
        //gradientBlock(glm::vec2(tpWidth-colorPickerWidth, y),
          //glm::vec2(colorPickerWidth, 2.7), colorDarkGray, colorDarkGray));
    //background->dim.start.z -= 0.1;

    r(result, button(glm::vec2(0,y), iconLeft, glm::vec2(tpWidth-colorPickerWidth-1,0.75),
          strdup_s(system->name), setCurrentLine, 0));
    y += 1.25;

    r(result, button(glm::vec2(0,y),
          line->flags & _transitEnabled ? iconCheck : iconNull,
          glm::vec2(tpWidth-colorPickerWidth-1, 0.75),
          strdup_s("Enabled"), toggleLineEnabled, 0));
    y += 1;

    r(result, label(glm::vec2(0,y), 0.75, strdup_s("Headway")));
    y += 0.75;
    for (int i = 0; i < 2; i++) {
      r(result, icon(glm::vec2(0,y), glm::vec2(0.75,0.75),
    ...
    for (int i=0; i <= tlS; i++) {
      Part* cursorHandle = r(sbox, block(glm::vec2(buttX,sy),
            glm::vec2(cursorWidth,0.2)));
      cursorHandle->onHover = hoverCursor;
      cursorHandle->onClick = setCursor;
      cursorHandle->itemData = i;

      if (i == cursor || i == lastHoveredCursor) {
        r(sbox, block(glm::vec2(buttX,sy-0.1), glm::vec2(0.1,0.4)));
        r(sbox, block(glm::vec2(buttX+cursorWidth-0.1,sy-0.1), glm::vec2(0.1,0.4)));
      } else {
      ...
      Stop* stop = getStop(stopNdx);
      Part* butt = button(glm::vec2(buttX,sy), iconBusSign, buttSize,
      ...
      r(sbox, button(glm::vec2(tpWidth-2.75,sy), iconEye,
            glm::vec2(0.75,0.75), focusStop, i));

      r(sbox, button(glm::vec2(tpWidth-1.75,sy), iconTrash,
            glm::vec2(0.75,0.75), removeStop, i));

      if (getSelectionType() == SelectionStop && stopNdx == getSelection()) {
        Part* arrow = r(sbox, icon(glm::vec2(tpWidth-3.75,sy),
              glm::vec2(0.75,0.75), iconLeft));
      ...
      if (i < line->legs.size()-1) {
        ...
        Part* timeLabel = r(sbox,
            label(glm::vec2(-0.1,sy+.45), 0.75, sprintf_o("+%s", dur)));
    ...
    r(result, scrollboxFrame(glm::vec2(0,y),
          glm::vec2(tpWidth, scrollBoxHeight), &stopsScroll, sbox));
  }
