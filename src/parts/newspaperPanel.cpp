...
Part* newspaperArticle(Column* column, NewspaperIssueArticle* article, float date) {
  ...
  Part* result = panel(column->loc, glm::vec2(column->width, 1));
  result->flags &= ~_partTextShadow;
  result->renderMode = RenderTransparent;

  glm::vec2 titleEnd(0,0);
  Part* titlePart = r(result, spanCenter(glm::vec2(0,0.5f), 0,
      glm::vec2(column->width, npTitleScale),
  ...
  Part* art = r(result, articleText(glm::vec2(0,titleEnd.y),
      glm::vec2(column->width, npScale),
      article->text, config));
  art->renderMode = RenderTransparent;

  float y = art->dim.end.y + titleEnd.y - 0.5f;
  result->dim.end = glm::vec3(column->width, y, 0);
  column->loc.y += y;
...
Part* newspaperAdvertisement(Column* column, item issue, item adNum, float targetSize) {
  ...
  bool ninetyThree = date >= (1993-c(CStartYear))*oneYear;
  std::string text = getNewspaperAdvertisement(issue, adNum);
  Part* result = panel(column->loc+glm::vec2(0,0.85f), glm::vec2(column->width, 1));
  result->flags &= ~_partTextShadow;
  result->renderMode = RenderTransparent;
  ...
  Part* art = r(result, articleText(glm::vec2(pd*2, pd*2),
      glm::vec2(column->width-pd*4, npScale),
  ...
  float bsy = y-pd;
  r(result, colorBlock(glm::vec2(0,0), glm::vec2(bsx, pd), config.textColor));
  r(result, colorBlock(glm::vec2(0,0), glm::vec2(pd, bsy), config.textColor));
  r(result, colorBlock(glm::vec2(0,bsy), glm::vec2(bsx, pd), config.textColor));
  r(result, colorBlock(glm::vec2(bsx-pd,0), glm::vec2(pd, bsy), config.textColor));

  return result;
}

Part* newspaperWidget(glm::vec2 bottomRight) {
  glm::vec2 size(3, 1);
//  glm::vec2 loc = bottomRight - size - glm::vec2(0, 0.5);
  glm::vec2 loc = glm::vec2(npWidth-size.x,-size.y-0.25f);
  ...
  r(widget, button(glm::vec2(2,0), iconX, glm::vec2(1,1),
        closeNewspaperPanel, 0));

  Part* backButt = r(widget, button(glm::vec2(0,0), iconLeft, glm::vec2(1,1),
        shiftIssue, -1));
  if (currentIssue <= 0) backButt->foregroundColor = PickerPalette::GrayLight;

  item numIssues = getNumNewspaperIssues();
  Part* foreButt = r(widget, button(glm::vec2(1,0), iconRight, glm::vec2(1,1),
        shiftIssue, 1));
...
std::vector<item> getArticleOrder() {
  std::vector<item> articleOrder;
  NewspaperIssue* issue = getNewspaperIssue(currentIssue);
...
Part* newspaperPanel() {
  ...
  r(result, newspaperWidget(glm::vec2(0, npWidth)));
  ...
  Part* scroll = scrollbox(glm::vec2(0,0), glm::vec2(npWidth, npHeight));
  float y = 0;

  std::string name = getNewspaperName();
  float nameWidth = stringWidth(name.c_str());
  float nameSpacing = (innerWidth)/nameWidth;
  Part* namePart = r(scroll, labelCenter(glm::vec2(scrollbarSpacing,0),
        glm::vec2(innerWidth, nameSpacing),
        strdup_s(name.c_str())));
  y += nameSpacing + 0.25f;

  r(scroll, blackBlock(glm::vec2(scrollbarSpacing, y),
        glm::vec2(innerWidth, dividerWidth)));
  y += 0.1 + dividerWidth;

  r(scroll, label(glm::vec2(scrollbarSpacing, y), subScl,
        printMoneyString(issue->price)));
  r(scroll, labelCenter(glm::vec2(scrollbarSpacing, y), glm::vec2(innerWidth, subScl),
        printDateString(issue->date)));
  float tempX = scrollbarSpacing+innerWidth-1 - (issue->weather != WeatherNull);
  r(scroll, labelRight(glm::vec2(tempX, y), glm::vec2(1, subScl),
        useMetric() ? sprintf_o("%2dC", int(issue->temp)) :
        sprintf_o("%2dF", int(issue->temp*1.8 + 32))));
  r(scroll, icon(glm::vec2(scrollbarSpacing+innerWidth-subScl, y),
        glm::vec2(subScl, subScl), getWeatherIconVec3(issue->weather)));
  y += subScl;
  ...
  columns.push_back(Column { glm::vec2(scrollbarSpacing, y), columnSize });
  columns.push_back(Column { glm::vec2(scrollbarSpacing+columnSize+colPad, y),
      columnSize*2+colPad });
  columns.push_back(Column { glm::vec2(scrollbarSpacing+(columnSize+colPad)*3, y),
      columnSize });

  // Sports
  item numSports = issue->sportsScores.size();
  glm::vec2 scoreSize(columns[0].width,npScale);
  glm::vec2 scoreMiddle(columns[0].width*.6f,npScale);
  ...
  for (int i = 0; i < numSports; i++) {
    ...
    r(scroll, blackBlock(columns[0].loc+glm::vec2(1,0),
          glm::vec2(columns[0].width-2, dividerWidth)));
    columns[0].loc.y += 0.1f + dividerWidth;
  }
  r(scroll, labelCenter(columns[0].loc, scoreSize,
        strdup_s("More Sports News on Page 4")));
  columns[0].loc.y += npScale;
  r(scroll, blackBlock(columns[0].loc, glm::vec2(columns[0].width, dividerWidth)));
  columns[0].loc.y += 0.1f + dividerWidth;
  ...
  r(scroll, button(columns[0].loc+glm::vec2(scoreSize.x-1,0), iconPin, glm::vec2(1,1),
  ...
  for (int i = 0; i < numFin; i++) {
    ...
    for (int j = 0; j < numPrevFin; j++) {
      ...
      Part* ico = r(scroll, icon(columns[0].loc+glm::vec2(columns[0].width*.65f,0),
            glm::vec2(npScale, npScale), diff < 0 ? iconDown : iconUp));
      if (ninetyThree) {
  ...
  // Articles
  item numArticles = issue->articles.size();
  std::vector<item> articleOrder = getArticleOrder();
  ...
  for (int i = 0; i < numArticles; i++) {
    ...
    if (numArticlesInCol[bestCol] > 0) {
      ...
      r(scroll, blackBlock(columns[bestCol].loc, glm::vec2(columns[bestCol].width, dividerWidth)));
    }
  ...
  for (int i = 1; i < columns.size(); i++) {
    ...
    r(scroll, blackBlock(glm::vec2(col1.loc.x-dividerXOff, colStartY),
          glm::vec2(dividerWidth, colEndy - colStartY - 0.5)));
  }
  ...
  Part* scrollFrame = scrollboxFrame(glm::vec2(-scrollbarSpacing, 0),
      glm::vec2(npWidth+scrollbarSpacing, npHeight),
