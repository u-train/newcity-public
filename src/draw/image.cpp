...
unsigned char *loadImage(const char *filename, int *x, int *y, int *n, int d) {
...
Image loadImage(const char *filename) {
  ...
  // SPDLOG_INFO("loadImage {} {}", filename, modFilename);
  result.data =
      stbi_load(modFilename.c_str(), &result.x, &result.y, &result.channels, 4);
  return result;
}
...
void freeImage(Image img) { stbi_image_free(img.data); }

void blitImage(Image source, Image dest, int x, int y);

glm::vec4 samplePixelExact(Image source, int x, int y) {
  ...
  if (ndx < 0 || ndx >= source.x * source.y * source.channels) {
    SPDLOG_INFO("samplePixelExact {}x{} at {},{}", source.x, source.y, x, y);
  }
  // glm::vec4 result(0,0,0,0);
  glm::vec4 result(float(source.data[ndx]), float(source.data[ndx + 1]),
...
glm::vec4 samplePixel(Image source, float x, float y) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    glm::vec4 sample = samplePixelExact(source, ix, iy);
    result += sample * fade;
  }
