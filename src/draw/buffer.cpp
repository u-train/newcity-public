...
static const GLsizei vec4Size = sizeof(glm::vec4);
static const int baseAttrib = 3;
static const int stride = 9;
static const int numABOFrames = 3;
static std::vector<ABO> abos[numABOFrames + 1];
static std::vector<ABO> treeABOs;
static std::unordered_map<item, item> treeABOmap;
static int currentABOFrame = 0;
static int currentABO = 0;
static std::vector<TreeData> treeDataToWriteFront, treeDataToWriteBack;
static int entitiesDrawn = 0;

DrawBuffer drawBuffers[2];
static std::atomic<bool> frontBuffer(false);
...
DrawCommand *nextDrawCommand(DrawBuffer *b, Shader s, item entity, item mesh,
                             item texture) {
  DBShaderPass *pass = &b->passes[s];
...
DrawCommand *getDrawCommand(DrawBuffer *b, Shader s, item entity, item mesh,
                            item texture) {
...
inline void collectEntity(item ndx, Cull cull, DrawBuffer *b) {
  ...
  __try {
    ...
    SPDLOG_ERROR("Windows Structured Exception in collectEntity for ndx {}",
                 ndx);
...
void collectEntities_g() {
  ...
  for (int i = 1; i <= numEntities; i++) {
    ...
    if (doSetEntityCulled)
      setEntityCulled_g(i, !found);
    if (found)
      nextCommand = ungroupedDrawCommands.next();
  }
  ...
  */
  ...
  if (c(CLogUpdateTime)) {
    SPDLOG_INFO("prep {:.2f}ms wait {:.2f}ms cull {:.2f}ms group {:.2f}ms",
                (prepEnd - prepStart) * 1000, (waitEnd - prepEnd) * 1000,
                (cullEnd - waitEnd) * 1000, (groupEnd - cullEnd) * 1000);
  }
}
...
void collectDrawBuffer() {
  ...
  if (isGameLoading() || c(CCullingAlgo) == 0) {
    ...
    for (int i = 1; i <= numEntities; i++) {
      ...
      if (found) {
        DrawCommand *command = getDrawCommand(
            b, (Shader)ungpd.shader, ungpd.entity, ungpd.mesh, ungpd.texture);
        memcpy_s(command->data.next(), &ungpd.data, sizeof(DrawData));
      }
    }
  }
...
#ifdef COUNT_ENTITIES
  for (int s = 0; s < numShaders; s++) {
    SPDLOG_INFO("{}/{} entities in {} shader", visEntPerShader[s],
                entitiesPerShader[s], getShaderName((Shader)s));
  }
#endif
}
...
void setupInstancedAttribs() {
  for (int i = 0; i < 4; i++) {
    ...
    glVertexAttribPointer(i + baseAttrib, 4, GL_FLOAT, GL_FALSE, dataSize,
                          (void *)(i * vec4Size));
    glVertexAttribDivisor(i + baseAttrib, 1);
  }

  glEnableVertexAttribArray(4 + baseAttrib);
  glVertexAttribIPointer(4 + baseAttrib, 2, GL_UNSIGNED_INT, dataSize,
                         (void *)(4 * vec4Size));
...
void setupTreeAttribs() {
  ...
  glVertexAttribPointer(0 + baseAttrib, 4, GL_FLOAT, GL_FALSE, vec4Size,
                        (void *)(0 * vec4Size));
...
glm::uvec2 convertFlags(uint64_t in) {
  uint32_t x = in >> 32;
  uint32_t y = in;
...
void draw(Shader s, DrawCommand *command) {
  int numInstances =
      command->numInstances >= 0 ? command->numInstances : command->data.size();
  bool isInstanced = isShaderInstanced(s);
  if (numInstances <= 0)
    return;
  if (isInstanced && command->aboID == 0)
    return;
  ...
  if (!(mesh->flags & _meshExists)) {
    SPDLOG_WARN("Trying to render non-existing mesh {}, flags {}, vbo {}, "
                "numT {}, numI {}",
                command->mesh, mesh->flags, mesh->vertexVBOID,
  ...
  if (isInstanced) {
    ...
    if (s == TreeShader || s == ShadowTreeShader) {
      ...
      glUniformMatrix4fv(getUniformID(s, MUniform), 1, GL_FALSE,
                         &data->matrix[0]);
      glm::uvec2 flags = glm::uvec2(data->dataFlags, data->flags);
      if (mesh->flags & _meshBillboard)
        flags.y |= _entityBillboard;
      glUniform2uiv(getUniformID(s, FlagsUniform), 1, (const GLuint *)&flags);
      setupTreeAttribs();
    } else {
    ...
    for (int i = 0; i < numInstances; i++) {
      ...
      glUniformMatrix4fv(getUniformID(s, MUniform), 1, GL_FALSE,
                         &dataI->matrix[0]);
      glm::uvec2 flags = glm::uvec2(dataI->dataFlags, dataI->flags);
      glUniform2uiv(getUniformID(s, FlagsUniform), 1, (const GLuint *)&flags);
...
ABO *bindABO() {
  std::vector<ABO> *fabos = &abos[currentABOFrame];
  ABO *result;
...
void bufferTreeABO(TreeData data) {
  ...
  glm::vec4 *vec4Data = valid ? data.data->get(0) : &fallback;
  int numInstances = valid ? data.data->size() : 0;
  int bufferSize = numInstances * sizeof(glm::vec4);
...
int numEntitiesDrawn() { return entitiesDrawn; }
