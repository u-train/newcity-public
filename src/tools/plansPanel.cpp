...
void plansPanel(Part* panel) {
  r(panel, label(glm::vec2(0,0), 1, strdup_s("Planner Mode")));
  float width = panel->dim.end.x - panel->padding*2;

  r(panel, button(glm::vec2(width-1,0), iconX, glm::vec2(1,1), togglePlanPanel, 0));

  if (getNumGraphPlans() > 0) {
    money cost = getTotalPlansCost();
    char* costStr = printMoneyString(cost);

    r(panel, superButton(glm::vec2(0.f, 1.5f), glm::vec2(width,1.2),
          iconYes, sprintf_o("Complete All for %s", costStr),
          buyAllPlans, 0, ActPlansComplete, false));
    free(costStr);

    r(panel, superButton(glm::vec2(0.f, 3.f), glm::vec2(width,1.2),
    ...
    r(panel, superButton(glm::vec2(0.f, 4.5f), glm::vec2(width,1.2),
          iconMessage, strdup_s("Plan Widgets"),
          toggleShowPlans, 0, ActPlansShowWidgets, isShowPlans()));

  } else {
    float ySize = 0;
    r(panel, multiline(glm::vec2(0,2), glm::vec2(width, 0.8),
