...
void compressBuffer(FileBuffer *buffer) {
  ...
  int finalCompSize = headerSize + LZ4_compress_default(uncompData, compDataPtr,
...
void decompressBuffer(FileBuffer *buffer) {
  ...
  int finalUncompSize =
      headerSize + LZ4_decompress_safe(compData, uncompDataPtr,
                                       compSize - headerSize,
                                       uncompSize - headerSize);
...
void startSegment(FileBuffer *buffer, const char *segmentName) {
  if (buffer->lastSegmentName != 0) {
    ...
    if (bytes > 1000000000) {
      ...
      SPDLOG_INFO("{:.2f} Mb    in Segment {}", bytes / 1000.f / 1000.f,
                  buffer->lastSegmentName);
    } else if (bytes > 1000) {
      SPDLOG_INFO("{:.2f} Kb    in Segment {}", bytes / 1000.f,
                  buffer->lastSegmentName);
    } else {
      SPDLOG_INFO("{} bytes in Segment {}", bytes, buffer->lastSegmentName);
    }
  }
...
FileBuffer readFromFile(const char *filename, bool winCompat) {
  ...
  */
  ...
  HANDLE fileHandle =
      CreateFileW(fn.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  // lpNumberOfBytesRead parameter can be NULL only if lpOverlapped parameter is
  // not NULL
  if (!(ReadFile(fileHandle, uCharBuf, lSize, &bytesRead, NULL))) {
    handleError("Error with ReadFile %s (bytesRead:%s)", fn.c_str(),
                std::to_string((unsigned long)bytesRead));
    CloseHandle(fileHandle);
    delete (uCharBuf);
    return buffer;
  }
  ...
  if (num != lSize) {
    handleError(
        sprintf_o("Error reading file (num: %d)(lSize: %d)", num, lSize));
  }
#endif
...
void fwrite_char(FileBuffer *out, char c) { fwrite(&c, sizeof(char), 1, out); }

void fwrite_int(FileBuffer *out, int num) { fwrite(&num, sizeof(int), 1, out); }

void fwrite_item(FileBuffer *out, item ndx) {
  fwrite(&ndx, sizeof(item), 1, out);
}

void fwrite_vec2(FileBuffer *out, glm::vec2 v) {
  fwrite(&v.x, sizeof(float), 1, out);
  fwrite(&v.y, sizeof(float), 1, out);
}

void fwrite_vec3(FileBuffer *out, glm::vec3 v) {
  fwrite(&v.x, sizeof(float), 1, out);
  fwrite(&v.y, sizeof(float), 1, out);
...
item fread_item(FileBuffer *in, int version) { return fread_item(in); }

glm::vec2 fread_vec2(FileBuffer *in) {
  glm::vec2 result;
...
glm::vec3 fread_vec3(FileBuffer *in) {
  glm::vec3 result;
  fread(&result.x, sizeof(float), 1, in);
  fread(&result.y, sizeof(float), 1, in);
...
char *fread_string_until_whitespace(FileBuffer *in) {
  ...
  for (;;) {
    ...
    if (isspace(c) || in->cursor >= in->length)
      break;
    nameBuf.push_back(c);
  }
  nameBuf.push_back('\0');
...
void fwrite_location_vector(FileBuffer *in, std::vector<Location> *list) {
...
void fread_location_vector(FileBuffer *in, std::vector<Location> *result,
                           int version) {
...
void fwrite_item_vector(FileBuffer *in, std::vector<item> *list) {
  fwrite_int(in, list->size());
  for (int i = 0; i < list->size(); i++) {
...
void fread_item_vector(FileBuffer *in, std::vector<item> *result, int version) {
  int num = fread_int(in);
  // if (num > 1000000 || num < 0) {
  // handleError("Error reading from file");
  //}
