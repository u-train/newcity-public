...
struct Font {
  ...
  float atlasHeight;
  float zeroWidth;
  glm::vec2 start[numCharacters];
  glm::vec2 offset[numCharacters];
  glm::vec2 size[numCharacters];
  glm::vec2 outerSize[numCharacters];
...
void parseFont() {
  std::string filenameStr = lookupFile("fonts/font.ttf", 0);
  ...
  float tallest = 0;
  glm::vec2 cursor(c(CTextAtlasPadding), c(CTextAtlasPadding));
  ...
  for (int i = 0; i < numCharacters; i++) {
    ...
    mainFont.size[i] = glm::vec2(width, height);
    mainFont.data[i] = data;
    mainFont.offset[i] = glm::vec2(g->bitmap_left, -(g->bitmap_top));

    if (cursor.x + width > c(CTextAtlasWidth) - c(CTextAtlasPadding)) {
      cursor = glm::vec2(c(CTextAtlasPadding),
  ...
  for (int i = 0; i < numCharacters; i++) {
    glm::vec2 start = mainFont.start[i];
    glm::vec2 size = mainFont.size[i];
    ...
    mainFont.size[i]+=8;
    mainFont.outerSize[i] = glm::vec2(mainFont.size[i])/c(CTextResolution);
...
void renderString(Mesh* mesh, const char* string, glm::vec3 start, glm::vec3 along,
  glm::vec3 down, float firstLineIndent) {

  if (string == 0) return;

  glm::vec3 cursor = start + firstLineIndent*along;
  glm::vec2 xHeight = glm::vec2(0, mainFont.xHeight);
  float fontSize = length(down);
  float padding = 0.1;
  glm::vec3 paddingX = fontSize*padding*normalize(along);
  glm::vec3 paddingY = fontSize*padding*normalize(down);
  glm::vec3 paddingXY = paddingX + paddingY;
  glm::vec3 texPadding = glm::vec3(c(CTextResolution)*padding/c(CTextAtlasWidth),
  ...
  for (int i = 0; string[i] != '\0'; i++) {
    ...
    glm::vec2 o = mainFont.offset[c] + xHeight;
    glm::vec3 os = glm::vec3(mainFont.outerSize[c],0);
    glm::vec3 l = glm::vec3(mainFont.start[c],0)-texPadding;
    glm::vec3 s = glm::vec3(mainFont.size[c],0)+texPadding*2.f;
    float a = characterWidth(c, string[i+1]);
    o.x += .5f * (a - os.x);

    glm::vec3 p = cursor + o.y * down + o.x * along - paddingXY;
    glm::vec3 iAlong = along * os.x + paddingX*2.f;
    glm::vec3 iDown  =  down * os.y + paddingY*2.f;
...
void renderString(Mesh* mesh, const char* string, glm::vec3 start, float size) {
  renderString(mesh, string, start, glm::vec3(size,0,0));
}

void renderString(Mesh* mesh, const char* string, glm::vec3 start, glm::vec3 along,
    float firstLineIndent) {
  glm::vec3 down = glm::vec3(-along.y, along.x, 0);
...
void renderString(Mesh* mesh, const char* string, glm::vec3 start, float size,
    float firstLineIndent) {
  renderString(mesh, string, start, glm::vec3(size,0,0), firstLineIndent);
}
...
glm::vec2 stringDimensions(const char* string) {
  if(string == 0) return glm::vec2(0,0);
  if(strlen(string) <= 0) return glm::vec2(0,0);
...
void renderStringCentered(Mesh* mesh, const char* string,
  ...
  glm::vec3 start = center - along * stringWidth(string) * 0.5f;
